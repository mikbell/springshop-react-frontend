...
export function ProductCard({
  ...
  return (
    <Card className="group pt-0 overflow-hidden rounded-xl transition-all duration-200 hover:shadow-md border-muted/60 flex flex-col h-full">
      ...
      <div className="relative aspect-4/3 overflow-hidden bg-muted">
        ...
        <div className="absolute right-3 top-3">
          <Button
            aria-label={isFavorite ? "Rimuovi dalla wishlist" : "Aggiungi alla wishlist"}
            ...
            type="button"
            variant="secondary"
            className="h-8 w-8 rounded-full shadow-sm backdrop-blur-md bg-background/80 hover:bg-background transition-transform active:scale-95"
          >
            <Heart
              className={cn(
                "h-4 w-4 transition-colors",
                isFavorite ? "fill-destructive text-destructive" : "text-muted-foreground group-hover:text-foreground"
              )}
            />
          </Button>
        </div>
      ...
      <CardHeader className="p-4 gap-1.5 flex-1">
      ...
      <CardFooter className="p-4 pt-0 justify-between items-center gap-2 mt-auto">
        <div className="flex flex-col">
          <span className="text-xs text-muted-foreground/70 font-medium">Prezzo</span>
